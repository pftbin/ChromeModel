...
extern BOOL  (*g_pSetWindowParam)(const CString& strUrl);
extern BOOL  (*g_pCreateChromeWindow)(CWnd* pParentWnd);
extern BOOL	 (*g_pDoModalChromeWindow)(CWnd* pParentWnd);
...
#if 1
	if (g_bCEFInitState && g_pDoModalChromeWindow)
	{
		BOOL bResult = TRUE;
		bResult &= g_pSetWindowParam(m_strInputUrl);
		if (bResult)
			bResult &= g_pDoModalChromeWindow(this);
	}
#else
	//
