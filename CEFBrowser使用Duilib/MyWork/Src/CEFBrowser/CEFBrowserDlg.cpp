...
CCEFBrowserDlg::CCEFBrowserDlg(CWnd* pParent /*=nullptr*/)
	: CDialogEx(IDD_CEFBROWSER_DIALOG, pParent)
{
...
BOOL CCEFBrowserDlg::OnInitDialog()
{
	...
	// 创建Dui窗口
	CEFWebkitBrowserWnd* pDuiWindow = new CEFWebkitBrowserWnd();
	pDuiWindow->Create(GetSafeHwnd(), _T("浏览器"), UI_WNDSTYLE_DIALOG, WS_EX_WINDOWEDGE);
	pDuiWindow->CenterWindow();//覆盖窗口
	pDuiWindow->ShowModal();
