...
public:
	...
	void RefreshEditingState()
	{
		...
		{
			// 发送消息到Render进程
			CefRefPtr<CefProcessMessage> replyMessage = CefProcessMessage::Create("ask_checksavestate");
			m_Browser->GetMainFrame()->SendProcessMessage(PID_RENDERER, replyMessage);
		}
	}
	bool IsEditing()
	{
	...
	virtual bool OnProcessMessageReceived(CefRefPtr<CefBrowser> browser, CefRefPtr<CefFrame> frame, CefProcessId source_process, CefRefPtr<CefProcessMessage> message) 
	{
		...
		if (messageName == "reply_checksavestate")
		{
			//参数解析
			CefRefPtr<CefListValue> args = message->GetArgumentList();
			CefString cefValue0 = args->GetString(0);
			CString strValue0; strValue0 = cefValue0.ToWString().c_str();
			if (strValue0 == _T("0"))
				m_bIsEditing = FALSE;
			else
				m_bIsEditing = TRUE;
